            ...
            <a href="#" class="d-flex align-items-center text-decoration-none dropdown-toggle" id="userDropdown"
                ...
                <div class="position-relative">
                    <img src="{{ auth()->user()->avatar ? asset('storage/' . auth()->user()->avatar) : asset('./assets/img/user.png') }}"
                        alt="Avatar de {{ auth()->user()->name }}" class="user-avatar rounded-circle shadow-sm"
                        width="50" height="50" style="object-fit: cover;">
                    <!-- Status Indicator -->
                    <span
                        class="position-absolute bottom-0 end-0 translate-middle p-1 bg-success border border-light rounded-circle"
                        style="width: 12px; height: 12px;" title="En línea">
                    </span>
                </div>
    ...
    <nav class="sidebar-menu" role="menu">
        @foreach ($navItems[auth()->user()->getRoleNames()->first()] ?? [] as $item)
            <a href="{{ route($item['route']) }}"
                class="nav-link {{ $currentRoute === $item['route'] ? 'active' : '' }}" role="menuitem"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Función para reposicionar dropdowns con debounce
                ...
                dropdowns.forEach(menu => {
                    ...
                    if (!toggle) {
                        ...
                        if (dropdownContainer) {
                            toggle = dropdownContainer.querySelector(
                    ...
                    if (toggle) {
                        ...
                        // Calcular posición solo si es necesario
                        const newLeft = sidebarWidth + 8;
                        let newTop = toggleRect.top;

                        // Ajustar si se sale de la pantalla por abajo
                        const viewportHeight = window.innerHeight;
                        const menuBottom = newTop + menuRect.height;
                        ...
                        if (Math.abs(currentLeft - newLeft) > 5 || Math.abs(currentTop -
                            newTop) > 5) {
                            menu.style.position = 'fixed';
                            menu.style.left = newLeft + 'px';
                            menu.style.top = newTop + 'px';
                            menu.style.zIndex = '1055';
                            menu.style.transition = 'left 0.2s ease, top 0.2s ease';
                        }
                    }
                });
            }, 10); // Pequeño debounce de 10ms
        }
        ...
        // Reposicionar al hacer scroll (con throttle)
        let scrollTimeout;
        window.addEventListener('scroll', function() {
            if (document.querySelectorAll('.sidebar .dropdown-menu.show').length > 0) {
                clearTimeout(scrollTimeout);
                scrollTimeout = setTimeout(repositionDropdowns, 50);
            }
        }, {
            passive: true
        });

        // Observar cambios en el sidebar para reposicionar dropdowns (con debounce)
        let observerTimeout;
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.attributeName === 'class') {
                    clearTimeout(observerTimeout);
                    observerTimeout = setTimeout(repositionDropdowns, 100);
                }
            });
        });
        ...
        document.addEventListener('click', function(event) {
            ...
            dropdowns.forEach(dropdown => {
                ...
                if (toggle && !dropdown.contains(event.target) && !toggle.contains(event
                    .target)) {
