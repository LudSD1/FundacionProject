<div class="subtema-content">
    <div class="subtema-header">
        ...
        <div class="modal fade" id="modalEditarSubtema-{{ $subtema->id }}" tabindex="-1" aria-labelledby="modalEditarSubtemaLabel-{{ $subtema->id }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    <div class="modal-header bg-gradient-success text-white">
                        <div class="modal-title-section">
                            ...
                            <div>
                                <h5 class="modal-title mb-0" id="modalEditarSubtemaLabel-{{ $subtema->id }}">Editar Subtema</h5>
                                <small class="opacity-75">{{ $subtema->titulo_subtema }}</small>
                            </div>
                        </div>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Cerrar"></button>
                    </div>
                    <form method="POST" action="{{ route('subtemas.update', encrypt($subtema->id)) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-4">
                                    <div class="image-upload-section">
                                        ...
                                        <div class="file-upload-wrapper mt-3">
                                            <input type="file" class="form-control form-control-modern" name="imagen"
                                                accept="image/*" id="subtemaImageUpload-{{ $subtema->id }}">
          ...
          @if (auth()->user()->hasRole('Docente') && $cursos->docente_id == auth()->user()->id)
            <div class="subtema-actions">
                <div class="action-buttons">
                    ...
                    <!-- Eliminar -->
                    <form class="d-inline"
                          action="{{ route('subtemas.delete', encrypt($subtema->id)) }}"
                          method="POST"
                    ...
                    <div class="info-card" data-resource-type="{{ $recurso->tipoRecurso }}">
                        ...
                        <div class="d-flex align-items-center gap-2 mt-2">
                            @if ($recurso->archivoRecurso)
                                <a href="{{ asset('storage/' . $recurso->archivoRecurso) }}" class="btn-action btn-action-view"
                                    target="_blank">
                            ...
                            @if (auth()->user()->hasRole('Docente') && $cursos->docente_id == auth()->user()->id)
                                <div class="teacher-actions d-inline-flex gap-1">
                                    ...
                                    <form action="{{ route('eliminarRecursosSubtemaPost', encrypt($recurso->id)) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este recurso?')">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <div class="modal fade" id="modalEditarRecurso-{{ $recurso->id }}" tabindex="-1" aria-labelledby="modalEditarRecursoLabel-{{ $recurso->id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-lg modal-dialog-centered">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form method="POST" action="{{ route('editarRecursosSubtemaPost', encrypt($recurso->id)) }}" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="mb-3">
                                                        <label class="form-label">Título del Recurso *</label>
                                                        <input type="text" name="tituloRecurso" class="form-control form-control-modern" value="{{ $recurso->nombreRecurso }}" required maxlength="255">
                                                    </div>
                                                    <div class="mb-3">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="tipoRecurso" class="form-select">
                                                            ...
                                                            <option value="drive" {{ $recurso->tipoRecurso === 'drive' ? 'selected' : '' }}>Google Drive</option>
                                                            ...
                                                            <option value="enlace" {{ $recurso->tipoRecurso === 'enlace' ? 'selected' : '' }}>Enlace externo</option>
                                                            <option value="archivos-adjuntos" {{ $recurso->tipoRecurso === 'archivos-adjuntos' ? 'selected' : '' }}>Archivos comprimidos</option>
                                                        </select>
                                                    </div>
                                                    <div class="mb-3">
                                                        ...
                                                        @if ($recurso->archivoRecurso)
                                                            <div class="mb-2">
                                                                <a href="{{ asset('storage/' . $recurso->archivoRecurso) }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                                                        ...
                                                        <div class="form-check mt-2">
                                                            <input class="form-check-input" type="checkbox" name="eliminarArchivo" id="eliminarArchivo-{{ $recurso->id }}" value="1">
                                                            <label class="form-check-label" for="eliminarArchivo-{{ $recurso->id }}">Eliminar archivo actual</label>
                                                        </div>
                                                    </div>
                        ...
                        <div class="mt-3">
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="modalCuestionarioLabel-{{ $actividad->id }}">
                                                        @if ($actividad->cuestionario)
                                                            Editar Cuestionario
                                                        @else
                                                            Crear Cuestionario
                                                    ...
                                                    <form method="POST"
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="mostrar_resultados" class="form-label">Mostrar
                                                                Resultados</label>
                                                            <select name="mostrar_resultados" class="form-select"
                                                                required>
                                                                <option value="1"
                                                        ...
                                                        <div class="mb-3">
                                                            <label for="max_intentos" class="form-label">Número Máximo de
                                                                Intentos</label>
                                                            <input type="number" name="max_intentos"
                                                                class="form-control"
                                                        ...
                                                        <!-- Tiempo Límite -->
                                                        <div class="mb-3">
                                                            <label for="tiempo_limite" class="form-label">Tiempo Límite
                                                                (en
                                                                minutos)</label>
                                                            <input type="number" name="tiempo_limite"
                                                                class="form-control"
                            ...
                            @if (auth()->user()->hasRole('Docente'))
                                ...
                                <div class="modal fade" id="modalEditarActividad-{{ $actividad->id }}"
                                    tabindex="-1" aria-labelledby="modalEditarActividadLabel-{{ $actividad->id }}"
                                    aria-hidden="true">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        ['modalEditarSubtema-','modalEditarActividad-','modalCuestionario-','modalActividad-','modalRecurso-','modalEditarRecurso-'].forEach(function(prefix){
