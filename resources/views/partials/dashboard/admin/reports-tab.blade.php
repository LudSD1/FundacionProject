@php
    ...
    $query = \App\Models\Cursos::select(
        'id',
        'nombreCurso',
        'codigoCurso',
        'docente_id',
        ...
        ->withAvg(
            [
                'inscritos as promedio_progreso' => function ($i) {
                    $i->whereNull('deleted_at');
                },
            ],
            'progreso',
        ...
        <div class="col-xl-2 col-md-4 col-6">
            <div class="rpt-metric-card shadow-sm">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="fw-bold fs-5 text-dark">{{ \App\Models\Cursos::where('tipo', 'congreso')->count() }}
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="rpt-metric-card shadow-sm">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="fw-bold fs-5 text-dark">
                        {{ number_format(\App\Models\Inscritos::avg('progreso') ?? 0, 1) }}%</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="ntf-toolbar">
        ...
        <form id="rptSearchForm" method="GET" action="" class="ntf-toolbar-search">
            ...
            <div class="tbl-hero-search">
                ...
                <input type="text" class="tbl-hero-search-input ntf-search-light" name="q" id="rptSearchInput"
                    value="{{ $q }}" placeholder="Buscar por nombre, código o instructor…"
                    autocomplete="off">
            </div>
        </form>
        ...
        <form id="rptFilterForm" method="GET" action="">
            ...
            <div class="tbl-hero-select-wrap ntf-select-light">
                ...
                <select name="estado" class="tbl-hero-select ntf-select-field" id="rptFilterSelect">
                    ...
                    <option value="Inactivo" {{ $estado === 'Inactivo' ? 'selected' : '' }}>Inactivos (Próximos)
                    </option>
        ...
        <table class="table-modern">
            ...
            <tbody>
                @forelse($reports as $curso)
                    ...
                    @endphp
                    <tr data-estado="{{ $estadoCurso }}" data-nombre="{{ strtolower($curso->nombreCurso) }}">

                        <td>
                            <div class="rpt-course-cell">
                                <img src="{{ $curso->imagen ? asset('storage/' . $curso->imagen) : asset('assets/img/bg2.png') }}"
                                    alt="{{ $curso->nombreCurso }}" class="rpt-thumb"
                                    onerror="this.src='{{ asset('assets/img/bg2.png') }}'">
                                <div class="overflow-hidden">
                                    <span class="course-name d-block text-truncate mb-0"
                                        title="{{ $curso->nombreCurso }}">
                                        {{ ucfirst(strtolower($curso->nombreCurso)) }}
                                    </span>
                                    <div class="d-flex align-items-center gap-2">
                                        <small class="text-muted fw-mono"
                                            style="font-size: 0.65rem;">{{ $curso->codigoCurso }}</small>
                                        @if ($isCongreso)
                                            <span class="rpt-stat-badge bg-info-subtle text-info"
                                        ...
                                        @if ($curso->precio > 0)
                                            <span class="rpt-stat-badge bg-light text-dark border">
                                                <i
                                                    class="bi bi-tag-fill text-primary"></i>Bs.{{ number_format($curso->precio, 0) }}
                                            </span>
                                        @else
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-3">
                                <div class="d-inline-flex flex-column align-items-center">
                                    ...
                                    <span class="rpt-stat-badge bg-primary-subtle text-primary"
                                        style="font-size: 0.6rem;" title="Total inscritos">
                                ...
                                @if ($isCongreso || $totalCertificados > 0)
                                    <div class="d-inline-flex flex-column align-items-center">
                                        ...
                                        <span class="rpt-stat-badge bg-warning-subtle text-warning"
                                            style="font-size: 0.6rem;" title="Certificados emitidos">
                        ...
                        <td>
                            <div class="teacher-cell">
                                ...
                                <div class="d-flex flex-column">
                                    <span
                                        class="small fw-medium text-dark">{{ $curso->docente->name ?? 'Sin asignar' }}</span>
                        ...
                        <td>
                            <div class="action-buttons-cell justify-content-center">
                                <a class="btn-action-modern btn-view" href="{{ route('rfc', encrypt($curso->id)) }}"
                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                    title="Ver estadísticas detalladas">
                                    <i class="bi bi-bar-chart-line-fill"></i>
                                </a>
                                <a class="btn-action-modern btn-edit"
                                    href="{{ route('editarCurso', encrypt($curso->id)) }}" data-bs-toggle="tooltip"
                                    data-bs-placement="top" title="Editar configuración">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <a class="btn-action-modern"
                                    style="background: #f8fafc; color: #64748b; border: 1px solid #e2e8f0;"
                                    href="{{ route('Curso', $curso->codigoCurso ?? $curso->id) }}" target="_blank"
                                    data-bs-toggle="tooltip" data-bs-placement="top" title="Ver en el aula">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state-table">
                                ...
                                @if ($q || $estado !== 'all')
                                    <a href="?tab=reportes" class="tbl-hero-btn tbl-hero-btn-primary"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('rptSearchInput')
                ?.addEventListener('input', function() {
                    ...
                    rptTimer = setTimeout(() =>
                        document.getElementById('rptSearchForm').submit(), 500);
                });
