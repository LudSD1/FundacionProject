                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" class="form-control modern-input" id="fecha_asistencia"
                                        name="fecha_asistencia" value="{{ now()->format('Y-m-d') }}"
                                            ...
                                            <div class="student-info">
                                                ...
                                                <div class="student-details">
                                                    ...
                                                    <small class="text-muted">email:
                                                        {{ $inscrito->estudiantes->email }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            ...
                                            <select name="asistencia[{{ $index }}][tipo_asistencia]"
                                                class="form-select attendance-select" data-row-index="{{ $index }}"
                                                @if ($cursos->fecha_fin && now() > $cursos->fecha_fin) disabled @endif>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-6 text-center">
                                <button type="submit" class="btn btn-primary btn-lg w-100" id="saveAttendanceBtn">
                                    ...
                                    <span id="saveButtonText">Guardar Asistencias
                                        (0/{{ $inscritos->where('cursos_id', $cursos->id)->count() }})</span>
                                </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateStatusDisplay(selectElement) {
                ...
                switch (selectElement.value) {
                    ...
                    case 'Licencia':
                        ...
                        '<i class="fas fa-info-circle me-1"></i><span>Con Licencia</span>';
                        break;
                    case 'Falta':
            ...
            attendanceForm.addEventListener('submit', function(e) {
                const selectedCount = Array.from(attendanceSelects).filter(select => select.value !== '')
                    .length;

                if (selectedCount === 0) {
                    e.preventDefault();
                    showAlert('Debe seleccionar al menos un tipo de asistencia antes de guardar.',
                        'warning');
                    return;
                }

                if (!confirm(
                    `¿Está seguro de registrar la asistencia para ${selectedCount} estudiante(s)?`)) {
                    e.preventDefault();
                    return;
                }
