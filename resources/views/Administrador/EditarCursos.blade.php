        ...
        <div class="tbl-card-hero">
            ...
            <div class="tbl-hero-controls text-end">
                ...
                <div class="text-white small mb-1">
                    Paso <span id="stepCounter">1</span> de <span id="totalStepsCounter">{{ auth()->user()->hasRole('Administrador') ? 5 : 4 }}</span>
                </div>
            ...
            <div class="p-4 p-md-5">
                ...
                <div class="form-step active" data-step="1">
                    ...
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            @if(auth()->user()->hasRole('Administrador'))
                                <div class="input-group">
                                    ...
                                    <input type="text" name="nombre" class="form-control bg-light" value="{{ old('nombre', $cursos->nombreCurso) }}" required>
                                </div>
                            @else
                        ...
                        <div class="col-12">
                            <label class="form-label fw-bold text-muted small text-uppercase">Descripción del Curso</label>
                            <textarea name="descripcion" id="descripcionTA" class="form-control bg-light" rows="4" maxlength="500" required placeholder="Describe los objetivos...">{{ old('descripcion', $cursos->descripcionC) }}</textarea>
                        ...
                        <div class="col-md-6">
                            ...
                            @if(auth()->user()->hasRole('Administrador'))
                                <select name="tipo" class="form-select bg-light">
                                    <option value="curso" {{ $cursos->tipo == 'curso' ? 'selected' : '' }}>📚 Curso Regular</option>
                ...
                <div class="form-step" data-step="3" style="display: none;">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-bold text-muted small text-uppercase">Visibilidad del Curso</label>
                            <select name="visibilidad" class="form-select bg-light">
                                <option value="publico" {{ $cursos->visibilidad == 'publico' ? 'selected' : '' }}>🌐 Público (Todos)</option>
                                <option value="privado" {{ $cursos->visibilidad == 'privado' ? 'selected' : '' }}>🔒 Privado (Restringido)</option>
                            </select>
                        </div>

                        <div class="col-md-4">
                            <label class="form-label fw-bold text-muted small text-uppercase">Duración Estimada (Horas)</label>
                            <input type="number" name="duracion" class="form-control bg-light" value="{{ old('duracion', $cursos->duracion) }}" min="1" required>
                        </div>
                        ...
                        <div class="col-md-4">
                            <label class="form-label fw-bold text-muted small text-uppercase">Precio de Inscripción (Bs)</label>
                            <div class="input-group">
                                <span class="input-group-text bg-light fw-bold text-primary">Bs</span>
                                <input type="number" name="precio" class="form-control bg-light" value="{{ old('precio', $cursos->precio) }}" step="0.01" min="0" required>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Step 4/3: Archivos -->
                <div class="form-step" data-step="{{ auth()->user()->hasRole('Administrador') ? 4 : 3 }}" style="display: none;">
                    <div class="step-header mb-4">
                        <h4 class="text-primary fw-bold mb-1">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <div class="ec-file-drop-area p-4 text-center border rounded-4 bg-light">
                                ...
                                <input type="file" name="archivo" accept=".pdf" id="archivoInput" class="form-control form-control-sm">
                            ...
                            @if($cursos->archivoContenidodelCurso)
                                <div class="mt-3 p-3 bg-white border rounded-3 d-flex align-items-center">
                                    ...
                                    <div class="overflow-hidden">
                                        ...
                                        <a href="{{ asset('storage/'.$cursos->archivoContenidodelCurso) }}" target="_blank" class="small fw-bold text-primary text-truncate d-block">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="ec-file-drop-area p-4 text-center border rounded-4 bg-light">
                                ...
                                <input type="file" name="imagen" accept="image/*" id="imagenInput" class="form-control form-control-sm">
                                <small class="text-muted mt-2 d-block">Imagen recomendada: 1200x800px</small>
                            </div>
                            @if($cursos->imagen)
                                <div class="mt-3 p-3 bg-white border rounded-3 d-flex align-items-center">
                                    <img src="{{ asset('storage/'.$cursos->imagen) }}" class="rounded border me-3" style="width: 50px; height: 50px; object-fit: cover;">
                                    <div>
                ...
                <!-- Step 5/4: Categorías -->
                <div class="form-step" data-step="{{ auth()->user()->hasRole('Administrador') ? 5 : 4 }}" style="display: none;">
                    <div class="step-header mb-4">
                        <h4 class="text-primary fw-bold mb-1">
                    ...
                    <div class="tbl-hero-search mb-4 shadow-sm border rounded-pill overflow-hidden bg-white">
                        ...
                        <input type="text" class="form-control border-0 bg-transparent py-2" id="buscarCat" placeholder="Filtrar categorías...">
                    </div>
                    ...
                    <div class="alert alert-info border-0 rounded-4 mt-4 py-2">
                        ...
                        <span class="small fw-bold"><span id="catCount">{{ $cursos->categorias->count() }}</span> categorías seleccionadas</span>
                    </div>
                </div>
...
<style>
    ...
    .ec-cat-grid {
        ...
        gap: 0.75rem; max-height: 300px; overflow-y: auto; padding: 0.5rem;
    }
    .ec-cat-item {
        padding: 0.75rem; border-radius: 12px; background: #fff;
        border: 1.5px solid #e2eaf4; cursor: pointer; transition: all 0.2s;
        display: flex; align-items: center; gap: 0.75rem;
    }
    .ec-cat-item:hover { border-color: #145da0; background: #f8fafc; }
    .ec-cat-item.checked { border-color: #145da0; background: rgba(20, 93, 160, 0.05); }
    .ec-cat-check {
    ...
    .ec-cat-name { font-size: 0.82rem; font-weight: 600; color: #334155; }
...
<script>
    ...
    function validateStep(step) {
        ...
        const inputs = el.querySelectorAll('input[required]:not(:disabled), select[required]:not(:disabled), textarea[required]:not(:disabled)');
    ...
    document.getElementById('wizardForm').addEventListener('submit', function(e) {
        ...
        if (cats === 0) {
            Swal.fire({ icon: 'warning', title: 'Categorías', text: 'Selecciona al menos una.' });
            return;
        }
        Swal.fire({
            title: '¿Guardar Cambios?', text: "Se actualizará la información.", icon: 'question',
            showCancelButton: true, confirmButtonColor: '#10b981', confirmButtonText: 'Sí, guardar'
